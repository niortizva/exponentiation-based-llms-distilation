\section{Proposed Methodology: Exponentiation of Non-Square Matrices}
...
\begin{proof}
  \begin{enumerate}
    ...
    \item[(iii)] \textbf{Null Element:} Let $\A \in \vectorspace{m}{n}$ and $\0{m}{n}$ be the zero non-square matrix ($\left(\0{m}{n}\right)_{i,j}= 0$), since all its entries are zero. Then $\left(\0{m}{n} \circledast \A\right)_{i,j} = 0$ for all $i, j$ since each term in the sum involves multiplication by zero. Thus, $\0{m}{n} \circledast \A = \0{m}{n}$.
  \end{enumerate}
\end{proof}
